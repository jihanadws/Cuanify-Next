...
export default function ManagementPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      try {
        await verifyAndCreateUser(user)
      } catch (error) {
      ...
      if (existingCategory) {
        setError(`Kategori "${formData.name}" sudah ada untuk ${formData.type === 'INCOME' ? 'pemasukan' : 'pengeluaran'}`)
        return
      }
      ...
      if (editingCategory) {
        ...
        if (insertError) {
          ...
          // If it's a foreign key constraint error, try to re-authenticate
          if (insertError.code === '23503' && insertError.message?.includes('categories_user_id_fkey')) {
            console.log('Foreign key constraint violation - attempting re-authentication')
            ...
            console.log('Re-authenticated user ID:', refreshedUser.id)
            setUser(refreshedUser)
            setError('Authentication refreshed. Please try again.')
            return
          }
  ...
  const deleteCategory = async (categoryId: string) => {
    ...
    const isDefaultCategory = categoryToDelete?.is_default
    
    const confirmMessage = isDefaultCategory 
      ? 'Anda akan menghapus kategori default sistem. Apakah Anda yakin ingin melanjutkan? Ini mungkin mempengaruhi transaksi yang ada.'
    ...
    try {
      ...
      if (error) {
        ...
        const successMessage = isDefaultCategory 
          ? 'Kategori default berhasil dihapus!'
              ...
              <div>
                ...
                {editingCategory?.is_default && (
                  <p style={{
                    ...
                    ⚠️ Anda sedang mengedit kategori default sistem
                  </p>
                )}
              </div>
              <button
                  ...
                  >
                    ...
                    <div style={{
                      ...
                      <button
                        ...
                        title={category.is_default ? "Edit kategori default" : "Edit kategori"}
                      >
                        ✏️
                      </button>
                      <button
                        ...
                        title={category.is_default ? "Hapus kategori default" : "Hapus kategori"}
                      >
                        🗑️
                      </button>
                    </div>
